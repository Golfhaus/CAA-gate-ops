...
displayLabel      what actually renders in the block. Usually == label.
                  Exception: for a waypoint-split RON, only the piece whose
                  time span actually contains the 03:00 axis instant (the
                  same test that decides whether a piece renders as one
                  segment or wraps into two) keeps the full "routeThis ->
                  routeNext" label -- every other piece in that split gets
                  an empty string (no center label at all; the corner
                  moveTo/moveFrom code already says where it's headed).
                  turn/ROD splits have no dual-route concept and are
                  unaffected -- every piece keeps its one route number.
...
`displayLabel`/`arrivalCity`/`departureCity`/`moveTo`/`moveFrom` are all new. **Backward
...
annotations. The on-chart label falls back the same way
(`claim.displayLabel !== undefined ? ... : claim.label`), so an old-format
claim just shows its original `label` unchanged. No version flag needed in
the data — old and new JSON files can sit side by side in `data/` and both
render correctly.
